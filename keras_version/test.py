...
class HRED():
    ...
    def build_encoder(self):
        ...
        LSTM_input = Embedding(input_dim=len(self.w2v.voc), output_dim=self.input_size,
                               weights=[self.w2v.get_embedding_matrix()], mask_zero=True, trainable=True)(
            encoder_inputs)
        for i in range(1):
            LSTM_input = LSTM(self.latent_dim, return_sequences=True, dropout=con.enc_dropout,
                              recurrent_dropout=con.enc_r_dropout)(LSTM_input)
        _, state_h, state_c = LSTM(self.latent_dim, return_state=True, dropout=con.enc_dropout,
                                   recurrent_dropout=con.enc_r_dropout)(LSTM_input)
    ...
    def build_decoder(self):
        ...
        LSTM_input = Embedding(input_dim=len(self.w2v.voc), output_dim=self.input_size,
            ...
            decoder_inputs)
        for i in range(1):
            LSTM_input = LSTM(self.latent_dim, return_sequences=True, dropout=con.dec_dropout,
                              recurrent_dropout=con.dec_r_dropout)(LSTM_input)
    ...
    def train_final_model(self, model, encoder_input_data, decoder_input_data, decoder_target_data, meta_hh, meta_hc):
        ver = False
        if (
        con.print1):  # (datetime.datetime.now() < datetime.datetime(2018,5,31,10,23)) or(datetime.datetime.now() > datetime.datetime(2018,6,30,8)) ):
            ver = True
        if (ver):
            print("Q: %s A: %s W: %s" % (
            self.w2v.vec_to_sentence(encoder_input_data[0]), self.w2v.vec_to_sentence(decoder_input_data[0]),
            self.w2v.vec_to_sentence([i for i, x in enumerate(decoder_target_data[0]) if x == 1])))
        ...
        if (len(encoder_input_data) > 1 and con.validation > 0):
            loss = model.fit([encoder_input_data, decoder_input_data, meta_hh, meta_hc], decoder_target_data,
                             batch_size=len(encoder_input_data), epochs=con.epochs, verbose=ver,
                             validation_split=con.validation)  # ,callbacks = [print_weights])#,validation_split=0.2)
        else:
            loss = model.fit([encoder_input_data, decoder_input_data, meta_hh, meta_hc], decoder_target_data,
                             batch_size=len(encoder_input_data), epochs=con.epochs, verbose=ver,
                             validation_split=0)  # ,callbacks = [print_weights])#,validation_split=0.2)
